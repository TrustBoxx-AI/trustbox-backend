...
var FUJI = ClientCapability.SUPPORTED_CHAIN_SELECTORS["avalanche-testnet-fuji"];
var evmClient = new ClientCapability(FUJI);
var httpClient = new ClientCapability2;
var jsonHeaders = {
...
var executeIntentHandler = handler(evmClient.logTrigger({
  ...
  evmClient.writeReport(runtime2, {
    receiver: runtime2.config.contracts.intentVault,
    report: reportResult
  });
  return result;
});
    ...
    for (const entity of pending) {
      ...
      const scoreRes = httpClient.sendRequest(nodeRuntime, {
        ...
        body: Buffer.from(JSON.stringify({
          ...
          hederaTopicId: runtime2.config.hedera.topicId
        })).toString("base64")
      });
    ...
    for (const agent of agents) {
      const probeRes = httpClient.sendRequest(nodeRuntime, {
        ...
        body: Buffer.from(JSON.stringify({
          ...
          contractAddr: runtime2.config.contracts.trustRegistry
        })).toString("base64")
      });
